...
if [[ "${newly_downloaded["01.Shell-CreatingModels-1st.sh"]:-0}" == "1" ]]; then

  cp -af "$create_model01_script" "${work_root}/$(basename "$create_model01_script").bak"
  sed -E -i "s/^num_models=.*/num_models=$n_models/" "$create_model01_script"
...
update_shell_2nd() {
  ...
  [[ "${newly_downloaded["$base_name"]:-0}" == "1" ]] || return 0

  local dir_add="${linux_parent}/Model0${model_idx}-2"
  local na="${TYPE2[$model_idx]}"

  cp -af "$fname" "${work_root}/$(basename "$fname").bak"
...
for shf in "$bg_dir"/*.sh; do
  ...
  [[ "${newly_downloaded["$shf_base"]:-0}" == "1" ]] || continue

  cp -af "$shf" "${work_root}/$(basename "$shf").bak"
  ...
  python - "$shf" "${mw_args[@]}" <<'PY_SYNC_COMPOSITION'
import re
import sys
