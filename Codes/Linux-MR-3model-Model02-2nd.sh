...
mkdir -p "${parent_directory}/MR-Model02-2nd"
file_path="${parent_directory}/MR-Model02-2nd"
cd "${file_path}"
...
cp "${parent_directory}/Model02-2"/*.pdb "${pdb_directory_2}"
...
mr2_dir="${parent_directory}/MR-Model02-2nd"
...
model02_2_src="${parent_directory}/Model02-2"
if [ -s "$best_out" ]; then
  while IFS= read -r base; do
    target="${model02_2_src}/${base}.pdb"
    if [ -f "$target" ]; then
      mv -f "$target" "${model02_2_src}/Model02best.pdb"
      break
    fi
  done < "$best_out"
fi
