...
mkdir -p "${parent_directory}/MR-Model03-1st"
file_path="${parent_directory}/MR-Model03-1st"
cd "${file_path}"
...
mkdir -p "${parent_directory}/Model03-2"
mr_dir="${parent_directory}/MR-Model03-1st"
model_dir_1="${parent_directory}/Model03-1"
model_dir_2="${parent_directory}/Model03-2"
shopt -s nullglob
three_dna_candidates=( "${model_dir_1}"/3DNA-??NA )
if [ ${#three_dna_candidates[@]} -ge 1 ]; then
  three_dna_dir="${three_dna_candidates[0]}"
  if [ -f "${three_dna_dir}/bp_step.par" ]; then
    cp -f "${three_dna_dir}/bp_step.par" "${model_dir_2}/"
  fi
fi
shopt -u nullglob
...
process_category () {
  ...
  awk -v kw="$KEYWORD" '{
    ...
    sub(/^[[:space:]]+/, "", s);
    sub(/[-[:space:]].*$/, "", s);
    if (s ~ /^[0-9]+$/) { printf "%.10f %.10f %s\n", n1, n2, s }
  }' "$INFILE" | sort -nr -k1,1 -k2,2 | head -2 | awk -v pfx="$PREFIX" '{print "bp_step_" pfx $3}' >> "${mr_dir}/results-good-num.txt"
  awk -v kw="$KEYWORD" '{
    s=$1; n1=$2+0; n2=$3+0;
    sub("^.*" kw, "", s);
    sub(/^[[:space:]]+/, "", s);
  ...
  awk -v kw="$KEYWORD" '{
    ...
    sub(/^[[:space:]]+/, "", s);
    sub(/[-[:space:]].*$/, "", s);
    if (s ~ /^[0-9]+$/) { p=n1*n2; printf "%.10f %s\n", p, s }
  }' "$INFILE" | sort -nr -k1,1 | head -3 | awk -v pfx="$PREFIX" '{print "bp_step_" pfx $2}' >> "${mr_dir}/results-good-num.txt"
